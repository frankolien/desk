...
struct TrackedWalletsStrip: View {
    let onOpen: (TrackedWallet) -> Void
    let onAdd: () -> Void

    private var tracked: TrackedWallets { .shared }
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                Text("Tracked wallets")
                    ...
                    .foregroundStyle(DeskColor.nightText.color)
                Spacer()
                Text(tracked.list.isEmpty ? "Any Monad wallet" : "\(tracked.list.count) of \(TrackedWallets.limit)")
                    .font(.system(size: 13, weight: .medium, design: .rounded))
                    .foregroundStyle(DeskColor.nightMuted.color)
            }
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 10) {
                    ForEach(tracked.list) { wallet in
                        Button { onOpen(wallet) } label: { card(wallet) }.buttonStyle(.plain)
                    }
                    Button(action: onAdd) {
                        VStack(spacing: 8) {
                            ...
                            Text(tracked.list.isEmpty ? "Track a wallet" : "Add")
                                .font(.system(size: 12, weight: .semibold, design: .rounded))
                                .foregroundStyle(DeskColor.nightText.color)
                        }
                        .frame(width: tracked.list.isEmpty ? 150 : 96, height: 96)
    ...
    private func card(_ wallet: TrackedWallet) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                TraderAvatar(address: wallet.address, size: 30)
            ...
            Spacer(minLength: 0)
            Text(wallet.firstBuysOnly ? "First buys · $\(Int(wallet.minUsd))+" : "Every trade · $\(Int(wallet.minUsd))+")
